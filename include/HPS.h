...
class HPS {
  private:
    ...
    vector<PlaneParams> _RG_region_plane_params{
        {}}; // region plane params, also align with up mentioned _RG_region_count
  ...
  public:
    ...
    ~HPS() {
        free(_BR_pixel_refine_min_distance_to_plane);
        _BR_pixel_refine_min_distance_to_plane = NULL;
    };

    void SetData(const cv::Mat &color_img, const Eigen::MatrixXf &X, const Eigen::MatrixXf &Y,
