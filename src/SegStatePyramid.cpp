...
/* SegStatePyramid中，把上层的状态写到下层对应位置
 * */
        ...
        for (int j = 0; j < _layers[i]._height; ++j) {
            for (int k = 0; k < _layers[i]._width; ++k) {
                if (_layers[i+1][j/2][k/2]._is_plane) {
                    _layers[i].SetPlaneParams(j, k, _layers[i + 1][j / 2][k / 2]._plane_params, _layers[i + 1][j / 2][k / 2]._original_ind);
                }
            }
        }
    }
...
inline void SegStatePyramid_layer::SetPlaneParams(int row, int col, const PlaneParams &params, const vector<int> &original_ind) {
