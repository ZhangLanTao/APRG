...
void HPS::SetData(const cv::Mat &color_img, const Eigen::MatrixXf &X, const Eigen::MatrixXf &Y,
                  const Eigen::MatrixXf &Z) {
    _color_img = color_img.clone();
    _points_sample_pyramid.SetData(X, Y, Z);
}

inline void HPS::_BuildPyramid() { _points_sample_pyramid.BuildPyramid(); }
...
void HPS::_RecursivePlaneSegment() {
    ...
    while (!wait_to_process.empty()) {
        ...
        // Retrieve the point cloud of the patch corresponding to the index. Writing it as a function requires using a
        // vector to return three matrices, but operations with vectors waste time.
        const int min_patch_size = (_params.img_height / _params.block_num_y) >> (_params.num_pyramid_layers - 1);
        int row                  = min_patch_size * (this_ind[0] / _params.block_num_x),
            col                  = min_patch_size * (this_ind[0] % _params.block_num_x);
        ...
        if (jumps > _params.PS_max_allowed_jump_cnt)
            goto NOT_PLANE_GO_NEXT_LAYER;
        ...
        //   int key = cv::waitKey();
        //   if (key == 's')
        //     FitPlane(sampled_X, sampled_Y, sampled_Z, true, true);
        // }

        if (_params.PS_plane_thresh_mode == PLANE_PATCH_THRESHMODE_ABSOLUTE_MSE and
            plane_param.MSE > _params.PS_plane_thresh)
            goto NOT_PLANE_GO_NEXT_LAYER;
        if (_params.PS_plane_thresh_mode == PLANE_PATCH_THRESHMODE_DISTANCE_RATIO and
            plane_param.MSE > _params.PS_plane_thresh *
                                  (plane_param.mean_x * plane_param.mean_x + plane_param.mean_y * plane_param.mean_y +
                                   plane_param.mean_z * plane_param.mean_z))
            goto NOT_PLANE_GO_NEXT_LAYER;

        // confirm this is a plane
        _seg_state_pyramid.AddPatch(this_ind, plane_param);
        continue;

    NOT_PLANE_GO_NEXT_LAYER:
...
void HPS::_PatchwiseRegionGrowing() {
    ...
    while (_RG_GetNextSeed(seed_ind)) { // _RG_GetNextSeed has the next seed, returns true, seed_ind is changed inplace.
...
GET_NEXT_ONE:
    ...
    if (col == _seg_state_pyramid[layer]._width) {
        ...
        if (row == _seg_state_pyramid[layer]._height) {
            row = 0;
            --layer;
            if (layer < 0) { // All seeds have been traversed.
                seed_ind.clear();
                return false;
            }
        }
    }
    // If the seed is already a plane (labeled) or it is not a plane, then continue to the next one.
    if (_seg_state_pyramid[seed_ind]._label != -1 or !_seg_state_pyramid[seed_ind]._is_plane)
        goto GET_NEXT_ONE;
    return true;
}
...
void HPS::_RG_RegionGrowingFrom(const vector<int> &seed_ind, int label) {
    ...
    while (done_count != wait_to_process.size()) {
        ...
        // const int min_patch_size = (_params.img_height / _params.block_num_y) >>
        // (_params.num_pyramid_layers - 1); auto size_scaled = min_patch_size <<
        // seed[0]; int row = seed[1], col = seed[2]; auto rect =
        // cv::Rect(size_scaled * col, size_scaled * row, size_scaled, size_scaled);
        ...
        for (auto &neighbor_ind : neighbor_inds) {
            ...
            if (IsCoplanar(this_plane_param, neighbor_plane_param, _params.RG_plane_merge_min_cos_angle,
                ...
                // show neighbor_ind for debug
                // size_scaled = min_patch_size << neighbor_ind[0];
                // row = neighbor_ind[1], col = neighbor_ind[2];
                // rect = cv::Rect(size_scaled * col, size_scaled * row, size_scaled,
                // size_scaled); cv::rectangle(img_copy, rect, cv::Scalar(255, 0, 0), 1); cv::imshow("img-copy",
                // img_copy);
            }
        }
    }
...
vector<vector<int>> HPS::_RG_GetNeighbors(const vector<int> &ind) {
    ...
    if (row == 0)
        up = false;
    if (row == _seg_state_pyramid[layer]._height - 1)
        down = false;
    if (col == 0)
        left = false;
    if (col == _seg_state_pyramid[layer]._width - 1)
        right = false;
    if (up) {
    ...
    if (down) {
        for (int col_temp = col_in_layer0; col_temp < col_in_layer0 + this_patch_size;) {
            ...
            const auto &serched_original_ind =
                _seg_state_pyramid[0][row_in_layer0 + this_patch_size][col_temp]._original_ind;
            if (_seg_state_pyramid[serched_original_ind]._RG_visited or
                _seg_state_pyramid[serched_original_ind]._label != -1) {
                col_temp += 1 << serched_original_ind[0];
                continue;
    ...
    if (right) {
        for (int row_temp = row_in_layer0; row_temp < row_in_layer0 + this_patch_size;) {
            ...
            const auto &serched_original_ind =
                _seg_state_pyramid[0][row_temp][col_in_layer0 + this_patch_size]._original_ind;
            if (_seg_state_pyramid[serched_original_ind]._RG_visited or
                _seg_state_pyramid[serched_original_ind]._label != -1) {
                row_temp += 1 << serched_original_ind[0];
                continue;
...
void HPS::_BoundaryRefine() {
    // record which patch has been refined by which region
    cv::Mat patch_already_refined_by_label = cv::Mat::zeros(_layer0_label_img.size(), CV_8UC1);
    // merge label, use lut to speed up
    cv::Mat change_label_lut = cv::Mat(1, 256, CV_8UC1);
    for (int i = 0; i < 256; i++) {
        change_label_lut.at<uchar>(0, i) = i;
    }

    // reset min distance, this is slow, better not to do this
    // std::memset(_BR_pixel_refine_min_distance_to_plane, 100, _params.img_height * _params.img_width * sizeof(float));
    ...
    for (int label = 1; label < _RG_region_count; ++label) {
        ...
        // get refine mask, only erode for speed up
        ...
        for (int cell_row = 0; cell_row < mask_diff.rows; ++cell_row) {
            for (int cell_col = 0; cell_col < mask_diff.cols; ++cell_col) {
                if (mask_diff.at<uchar>(cell_row, cell_col) != 0) {
                    ...
                    if (l != 0) { // if this patch has been refined by another region, try to merge two regions
                        auto param1 = _RG_region_plane_params[l];
                        auto param2 = _RG_region_plane_params[label];
                        if (IsCoplanar(param1, param2, _params.RG_plane_merge_min_cos_angle,
                                       _params.RG_plane_merge_thresh_mode, _params.RG_plane_merge_max_distance2)) {
                            uchar target, from;
                        ...
                        patch_already_refined_by_label.at<uchar>(cell_row, cell_col) = label;

                    // calculate each pixel distance to region plane
                    auto X = _points_sample_pyramid.GetSampledPoints(min_patch_size * cell_row,
                                                                     min_patch_size * cell_col, 0, 0, min_patch_size);
                    auto Y = _points_sample_pyramid.GetSampledPoints(min_patch_size * cell_row,
                                                                     min_patch_size * cell_col, 0, 1, min_patch_size);
                    auto Z = _points_sample_pyramid.GetSampledPoints(min_patch_size * cell_row,
                                                                     min_patch_size * cell_col, 0, 2, min_patch_size);

                    auto distances_cell_stacked = (X * nx + Y * ny + Z * nz).array() + d;

                    // assign label to pixels
    ...
    // combine patch label and pixel label to get final label
    cv::resize(_layer0_label_img_eroded, _layer0_label_img_eroded_scaleup,
               cv::Size(_params.img_width, _params.img_height), 0, 0, cv::INTER_NEAREST);
    _final_label_img.setTo(0, _layer0_label_img_eroded_scaleup != 0);
    _final_label_img += _layer0_label_img_eroded_scaleup;

    // the lut is a chain, for example: a->b, b->c, c->d, then it should be a->d, b->d, c->d
    // so we need to find the final lut
    ...
    // to merge coplanar regions, change label
